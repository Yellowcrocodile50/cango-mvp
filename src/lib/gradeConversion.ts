 ...
 * 구간의 기울기로 외삽한다.
 *
 * 앵커의 5등급제 값은 9등급제 기준 오름차순에서 반드시 단조 비감소여야 한다(등급이
 * 나빠질수록 환산값도 나빠져야 하므로). 실측 노이즈로 인한 국소 역전(예: 3.72↔3.84,
 * 4.6↔4.71 구간)은 두 값을 뒤바꿔 단조성을 회복시켰다 — 이렇게 하지 않으면 역방향
 * (5→9) 보간이 해당 구간에서 튀어 필요 평균 환산값이 비정상적으로 나온다.
 */
